...
class NetworkService {
  ...
  Future<OrderDTO> acceptOrder(int orderId, double lat, double lng) async {
    final response = await _dioWrapper.sendRequest(
      ...
      method: NetworkMethod.post,
      formData: FormData.fromMap(
          {"order_id": orderId, "start_lat": lat, "start_lng": lng}),
    );
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (response.data['success'] == true) {
          return OrderCheck.fromJson(response.data);
          log("${response.data["message"]}");
        } else {
          // Если сервер вернул неуспешный ответ
          throw Exception(
              response.data['message'] ?? 'Ошибка при проверке заказа');
        }
      } else {
        // Если получен некорректный статус код
        throw Exception('Ошибка при проверке заказа: ${response.statusCode}');
      }
    } catch (e) {
      // Обработка ошибок, которые могут возникнуть при выполнении запроса
      print('Ошибка при выполнении запроса: $e');
      throw Exception('Произошла ошибка при проверке заказа');
    }
  }
  ...
  Future<Signed> ttnOrderStatus(int orderId, int userId) async {
    try {
      ...
      if (response.statusCode == 200) {
        // Парсим JSON-ответ и создаем объект модели Signed
        return Signed.fromJson(response.data);
      } else {
        // В случае ошибки возвращаем пустой объект модели Signed
        return Signed(signed: false);
      }
    } catch (e) {
      // Обработка ошибок при обращении к API
      print('Error occurred while checking TTN status: $e');

...
import 'package:europharm_flutter/network/models/dto_models/response/order_check_response.dart';
...
class GlobalRepository {
  ...
  Future<OrderDTO> acceptOrder(int orderId, double lat, double lng) async =>
      _networkService.acceptOrder(orderId, lat, lng);
  ...
  Future<Signed> ttnOrderStatus(int orderId, int userId) async {
    try {
      // Вызываем метод ttnOrderStatus из NetworkService
      final Signed signed =
          await _networkService.ttnOrderStatus(orderId, userId);
      return signed;
    } catch (e) {
      // Обработка ошибок при вызове метода ttnOrderStatus
      print("Error checking TTN status: $e");
      // Возвращаем пустой объект модели Signed в случае ошибки
      return Signed(signed: false);
    }
  }

  Future<PhoneList> getPhoneList({required int pointId}) async {
    try {
      final response = await _networkService.getPhoneList(pointId: pointId);
      return response;
    } catch (e) {
      print("Error getting phone list: $e");
      rethrow;
    }
  }
  ...
  Future<CreateCodeResponse> createCode({required int employeeId}) async {
    try {
      final response = await _networkService.createCode(employeeId: employeeId);
      // Проверяем успешность операции
      if (response.statusCode == 200 && response.success) {
        // Операция выполнена успешно, возвращаем объект с данными
        return response;
      } else {
        // Ошибка при создании кода, выбрасываем исключение
        throw Exception('Failed to create code: ${response.message}');
      }
    } catch (e) {
      // Ошибка при выполнении запроса или обработке ответа, выбрасываем исключение
  ...
  Future<ConfirmCodeResponse> confirmCode({required String code}) async {
    try {
      final response = await _networkService.confirmCode(code: code);
      return response;
    } catch (e) {
  ...
  Future<OrderCheck> orderCheck(int orderId) async {
    try {
      return await _networkService.orderCheck(orderId);
    } catch (e) {
      // Обработка ошибок, которые могут возникнуть при вызове метода orderCheck
      print('Ошибка при вызове метода orderCheck: $e');
      rethrow;
    }
  }
}

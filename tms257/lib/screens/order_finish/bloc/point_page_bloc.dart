...
class PointPageBloc extends Bloc<PointPageEvent, PointPageState> {
  PointPageBloc({
    ...
    // on<PointPageEventPhoneList>(_phoneList);
  }
  ...
  Future<void> _finishingPoint(
    ...
    try {
      if (order.orderType == 1) {
        ...
        emit(PointPageStateCheckInProgress());
        final orderCheckResponse = await repository.orderCheck(order.id);
        ...
        if (orderCheckResponse.data!.complete = true) {
          ...
          emit(PointPageStateCheckComplete(complete: false));
  ...
  Future<void> _phoneList(
    PointPageEventPhoneList event,
    Emitter<PointPageState> emit,
  ) async {
    emit(PointPageStateLoading());

    try {
      final phoneList = await repository.getPhoneList(pointId: event.pointId);
      print('Сработало!');
    } catch (e) {
      // Обрабатываем ошибку, если что-то пошло не так при получении списка номеров телефона
      emit(PointPageStateError(
          error: AppError(message: 'Ошибка при получении списка номеров: $e')));
    }
  }
}
//

                      ...
                      GestureDetector(
                        onTap: () {
                          AppRouter.push(
                            ...
                            ScanBarcodePage(
                              pointPageBloc:
                              BlocProvider.of<PointPageBloc>(context), repository: context.read<GlobalRepository>(),
                            ),
                          );
                        },
                        child: Container(
...
void showBarcodeEnterDialog(BuildContext context, Function onScan, GlobalRepository repository) {
  showAppBottomSheet(
    context,
    initialChildSize: 0.45,
    useRootNavigator: true,
    child: BuildBarcodeEnterField(onScan: onScan, repository: repository),
  );
}
...
class BuildBarcodeEnterField extends StatefulWidget {
  ...
  const BuildBarcodeEnterField({Key? key, required this.onScan, required this.repository})
      : super(key: key);
...
class _BuildBarcodeEnterField extends State<BuildBarcodeEnterField> {
  ...
  Future<void> _sendRequestToServer(String code) async {
    try {
        ...
        children: [
          ...
          MainButton(
            ...
            onTap: () {
              ...
              _sendRequestToServer(code); // Вызов метода для отправки запроса на сервер
            },
          )
        ],
      ),

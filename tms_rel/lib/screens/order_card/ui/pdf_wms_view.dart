...
class PDFWmsViewPage extends StatefulWidget {
  final List<int> pdfBytes;
  final OrderDTO order;

  PDFWmsViewPage({required this.pdfBytes, required this.order});

  @override
  _PDFViewPageState createState() => _PDFViewPageState();
}

class _PDFViewPageState extends State<PDFWmsViewPage> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) =>
          OrderDetailBloc(repository: context.read<GlobalRepository>()),
      child: BlocConsumer<OrderDetailBloc, OrderDetailState>(
        listener: (context, state) {
          if (state is OrdeWmsDocumentsIsLoadedSignState) {
                          ...
                          FloatingActionButton(
                            onPressed: () async {
                              if (currentPage! > 0) {
                                ...
                                int newPage =
                                    await _pdfController.getCurrentPage() ?? 0;
                                setState(() {
                                  currentPage = newPage;
                                });
                              }
